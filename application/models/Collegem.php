...
class Collegem extends CI_Model{
    ...
    function get_college($name){
            $sql='select course_name,duration,fees,qualification,university_name,stream_name from (SELECT * FROM `college_courses` WHERE `college_id` IN (select college_id from college where college_name = "'.$name.'")) c JOIN college cc ON c.college_id = cc.college_id JOIN courses ccc ON c.course_id = ccc.course_id JOIN university u ON cc.university_id = u.university_id JOIN streams s ON ccc.stream_id = s.stream_id order by stream_name';
            $query = $this->db->query($sql);
            $x = 1;
